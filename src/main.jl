...
using Printf  # フォーマット付き文字列を出力するためのライブラリ
using Dates   # 日付と時間を扱うためのライブラリ

#----------------------------------------------------------------------------------------------------#

include("config.jl")  # 設定ファイルをインクルード
include("savedata.jl")  # データ保存用のファイルをインクルード
include("me.jl")  # MAP-Elitesアルゴリズムの実装ファイルをインクルード
include("logger.jl")  # ログ出力用のファイルをインクルード
...
function main()
    ...
    MakeFiles()  # 結果ディレクトリとログファイルを作成
    
    # Check dimension
    if D == 2
        logger("WARN", "Dimension is default value \"2\"")  # 次元がデフォルト値「2」であることを警告
    elseif D <= 0
        logger("ERROR", "Dimension is invalid")  # 次元が無効であることをエラーログに記録
        exit(1)  # プログラムを終了
    else
    ...
    # Save result
    SaveResult(arch, iter_time, elapsed_time)  # 結果を保存
end
...
try
    global exit_code = 0  # グローバル変数exit_codeを初期化
