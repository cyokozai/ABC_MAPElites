...
function DE(population::Population, archive::Archive)
    ...
    for i in 1:N
        while r1 == r2 || r1 == r3 || r2 == r3 || I_a[r1].genes == I_p[i].genes || I_a[r2].genes == I_p[i].genes || I_a[r3].genes == I_p[i].genes
            r1, r2, r3 = rand(RNG, keys(I_a), 3)  # ランダムな異なるインデックスを生成
        end
        
        v = clamp.(I_a[r1].genes .+ F .* (I_a[r2].genes .- I_a[r3].genes), LOW, UPP)  # 差分ベクトルを計算

        u = crossover(I_p[i].genes, v)  # 交叉を行う
        ...
        if b[fit_index] < I_a[r1].benchmark[fit_index]
            archive.individuals[r1] = Individual(deepcopy(u), b, devide_gene(u))    # アーカイブを更新
        end

        if b[fit_index] < I_a[r2].benchmark[fit_index]
            archive.individuals[r2] = Individual(deepcopy(u), b, devide_gene(u))    # アーカイブを更新
        end
        
        if b[fit_index] < I_a[r3].benchmark[fit_index]
            archive.individuals[r3] = Individual(deepcopy(u), b, devide_gene(u))    # アーカイブを更新
        end
